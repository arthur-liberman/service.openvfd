...
class vfdDev:
	...
	def enableDisplay(self, value):
		self.__writeVfd(self._VFD_IOC_POWER, int(value))

	def getBrightness(self):
		return self.__readVfd(self._VFD_IOC_GBRIGHT)

	def setBrightness(self, value):
		self.__writeVfd(self._VFD_IOC_SBRIGHT, value)

	def getDisplayType(self):
		return self.__readVfd(self._VFD_IOC_GDISPLAY_TYPE)

	def setDisplayType(self, value):
		self.__writeVfd(self._VFD_IOC_SDISPLAY_TYPE, value)

	def setCharacterOrder(self, value):
		pack = struct.pack('BBBBBBB', value[0], value[1], value[2], value[3], value[4], value[5], value[6])
		self.__writeVfd(self._VFD_IOC_SCHARS_ORDER, pack, True)
	...
	def __readVfd(self, cmd, isBuf = False):
		...
		if (ioctl.DIR(cmd) == ioctl.READ and self.__writeVfd(cmd, 0)):
			...
			if (not isBuf and ret != None):
				ret = int(ret, 0)
		kodiLog('vfdDev.__readVfd: value = {0}'.format(str(ret)))
		return ret

	def __writeVfd(self, cmd, value, isBuf = False):
		ret = False
		if (os.path.isfile(_led_cmd)):
			...
			kodiLog('vfdDev.__writeVfd: value = {0}'.format(repr(value)))
			try:
